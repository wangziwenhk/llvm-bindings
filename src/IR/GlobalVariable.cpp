...
void GlobalVariable::Init(Napi::Env env, Napi::Object &exports) {
    ...
    const Napi::Function func = DefineClass(env, "GlobalVariable", {
...
GlobalVariable::GlobalVariable(const Napi::CallbackInfo &info) : ObjectWrap(info) {
    ...
    if (argsLen >= 1 && info[0].IsExternal()) {
        const auto external = info[0].As<Napi::External<llvm::GlobalVariable>>();
        ...
        if (argsLen == 3 ||
            ...
            const bool isConstant = info[1].As<Napi::Boolean>();
            const llvm::GlobalValue::LinkageTypes linkage = static_cast<llvm::GlobalValue::LinkageTypes>(info[2].As<Napi::Number>().Uint32Value());
            llvm::Constant *initializer = argsLen >= 4 ? Constant::Extract(info[3]) : nullptr;
            const std::string name = argsLen >= 5 ? std::string(info[4].As<Napi::String>()) : "";
        ...
        if (argsLen == 5 || argsLen >= 6 && info[5].IsString()) {
            ...
            const bool isConstant = info[2].As<Napi::Boolean>();
            const llvm::GlobalValue::LinkageTypes linkage = static_cast<llvm::GlobalValue::LinkageTypes>(info[3].As<Napi::Number>().Uint32Value());
            llvm::Constant *initializer = Constant::Extract(info[4]);
            const std::string name = argsLen >= 6 ? std::string(info[5].As<Napi::String>()) : "";
