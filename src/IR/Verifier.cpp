...
static Napi::Value verifyFunction(const Napi::CallbackInfo &info) {
    ...
    const llvm::Function *func = Function::Extract(info[0]);
    const bool result = llvm::verifyFunction(*func, &llvm::errs());
...
static Napi::Value verifyModule(const Napi::CallbackInfo &info) {
    ...
    const bool result = llvm::verifyModule(*module, &llvm::errs());
    return Napi::Boolean::New(env, result);
}
